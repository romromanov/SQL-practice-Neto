...
select s2.store_id, count(customer_id), c2.city, concat(s.first_name,' ' ,s.last_name)
from city c2 
...
group by s2.store_id, c2.city_id, s.staff_id
having count(customer_id) > 300

/*Задание №3
Для каждого фильма посчитайте сколько раз его брали в прокат, при этом работать нужно только с теми фильмами, в которых снимались актрисы 
с именем Julia.
В результирующей таблице должны быть следующие столбцы: Название фильма, количество аренд.*/
select f2.title, count(*)
from 
	(select f.film_id from actor a 
	join film_actor fa on a.actor_id = fa.actor_id
	join film f on f.film_id = fa.film_id
	where first_name ilike 'Julia'
	group by f.film_id) as fj
join film f2 on fj.film_id = f2.film_id 
...
from 
	(select film_id, ARRAY_AGG(category_id) as fcag
	from film_category
	group by film_id) as fca
join category c on c.category_id = any (fcag)
join film f on f.film_id = fca.film_id
...
join
	(select film_id, ARRAY_AGG(category_id) as fcag
	from film_category
	group by film_id) as fca on f.film_id = fca.film_id
join category c on c.category_id = any (fcag)
